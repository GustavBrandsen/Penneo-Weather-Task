...
- **SQLite**
  Easy to use, no extra setup, and enough for one write per minute, while it doesn't add unnecessary complexity.
...
- **Retries only for temporary errors**
  Retries for network errors, rate limits, and server errors.
  It does not retry invalid API keys or unknown cities.
- **API key from environment**
  The key comes from `.env` and is never hardcoded, in order to protect the API key.
